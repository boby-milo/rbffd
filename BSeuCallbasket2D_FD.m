function [u,err,tim,x,dx,N,W]=BSeuCallbasket2D_FD(Nx,M)
...
%% Initial condition
ii=1;
u0=max(0.5*(xvec'+yvec')-K,zeros(1,length(xvec)));
u=u0';
...
%Far-field BC
u(indff)=0.5*(xvec(indff)+yvec(indff))-K*exp(-r*dt);
...
for ii=3:M
    ...
    %Far-field BC
    u(indff)=0.5*(xvec(indff)+yvec(indff))-K*exp(-r*(ii-1)*dt);
