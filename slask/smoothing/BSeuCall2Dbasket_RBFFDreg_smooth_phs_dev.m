function [u,err,tim,x,dx,n,N,W] = BSeuCall2Dbasket_RBFFDreg_smooth_phs_dev(Nx,p,d,nm,M,Kmul)
%% 2D EU Call RBF-FD with BDF2

funname = 'BSeuCall2Dbasket_RBFFDreg_smooth_phs_dev';
...
if ~isempty(names)
    ...
    for ii = 1:length(xvec)
        ...
        if abs(xvec(ii)+yvec(ii)-2*Kx)/sqrt(2) <= 12*dx
            indreg = [indreg ii];
        end
    end
    hlocind = hloc(indreg);
    xvecind = xvec(indreg);
    yvecind = yvec(indreg);
%     uind = smooth4([xvecind', yvecind'],fu,dx,2); % dx or hloc
    uind = smooth([xvecind', yvecind'],fu,dx);
